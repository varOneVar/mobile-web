...
export default {
  ...
  methods: {
    // 点击图片重新绘制
    handleDraw() {
      const verify = this.draw()
      this.validate()
      this.$emit('getImgCode', verify)
    },
    // 1.随机数
    randomNum(min, max) {
    ...
    $_onPwdBlur(name) { // 密码失焦
      ...
      this.$emit('pwdBlur', this.validate())
    },
    validate() {
      console.log(this.verify, this.imgCode)
      if (!(this.verify && this.verify.toUpperCase() === this.imgCode)) {
        this.errorInfo = '图形验证码不正确'
        return false
      } else {
        this.errorInfo = ''
        return true
